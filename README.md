# Olá, eu sou o João! 👋

Sou um desenvolvedor apaixonado por criar soluções que unam uma boa experiência de usuário a uma arquitetura sólida e escalável. Atualmente, foco meu trabalho no ecossistema de JavaScript/TypeScript, transitando entre a construção de interfaces dinâmicas e o desenvolvimento de sistemas de back-end eficientes.

---

## 🛠️ Tecnologias e Ferramentas

Tenho trabalhado em projetos utilizando as seguintes tecnologias:

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![NestJS](https://img.shields.io/badge/nestjs-E0234E?style=for-the-badge&logo=nestjs&logoColor=white)
![Docker](https://img.shields.io/badge/docker-%230db7ed.svg?style=for-the-badge&logo=docker&logoColor=white)
![TypeScript](https://img.shields.io/badge/typescript-%23007acc.svg?style=for-the-badge&logo=typescript&logoColor=white)

- **Front-end:** Desenvolvimento de SPAs modernas e componentização com **React**.
- **Back-end:** Construção de APIs robustas e modulares utilizando **NestJS**.
- **Infraestrutura:** Uso de **Docker** para garantir ambientes de desenvolvimento padronizados e deploys mais seguros.

---



<!-- <picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/dev-joaoCintra1/dev-joaoCintra1/output/pacman-contribution-graph-dark.svg">
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/dev-joaoCintra1/dev-joaoCintra1/output/pacman-contribution-graph.svg">
  <img alt="Pac-Man contribution graph" src="https://raw.githubusercontent.com/dev-joaoCintra1/dev-joaoCintra1/output/pacman-contribution-graph.svg">
</picture> -->



## 📫 Vamos conversar?

Estou sempre aberto a trocar ideias sobre tecnologia, arquitetura de software ou possíveis colaborações. Fique à vontade para me chamar no LinkedIn:

[![LinkedIn](https://img.shields.io/badge/linkedin-%230077B5.svg?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/[SEU-LINKEDIN])

---
*“Acredito que o aprendizado contínuo é a melhor ferramenta para um desenvolvedor.”*
